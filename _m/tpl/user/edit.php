...
<div class="formbody">
    ...
    <form action="<?php echo _u('//edit/');?>" method="post" id="form1">
        <ul class="forminfo">
            ...
            <li>
                <label>相关证照</label>
                <?php if (!empty($_['company']['shop_image'])) echo '<a href="'._resize($_['company']['shop_image']).'" target="_blank"><img src="'._resize($_['company']['shop_image'], 60, 60).'" /></a>';?>
                <?php if (!empty($_['company']['business_image'])) echo '<a href="'._resize($_['company']['business_image']).'" target="_blank"><img src="'._resize($_['company']['business_image'], 60, 60).'" /></a>';?>
                <?php if (!empty($_['company']['tax_image'])) echo '<a href="'._resize($_['company']['tax_image']).'" target="_blank"><img src="'._resize($_['company']['tax_image'], 60, 60).'" /></a>';?>
                <?php if (!empty($_['company']['id_image'])) echo '<a href="'._resize($_['company']['id_image']).'" target="_blank"><img src="'._resize($_['company']['id_image'], 60, 60).'" /></a>';?>
            </li>
